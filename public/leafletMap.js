...
Promise.all([
    ...
    // get_data("SELECT * FROM nasafirmdata order by acq_date")
]).then(([provinceData]) => {
    // let coordinates = dumb_data("SELECT * FROM nasafirmdata order by acq_date")
    fetch('/getdata', {
        method:'POST',
    ...
    // let data = get_data('SELECT * FROM nasafirmdata order by acq_date')
    // console.log(data)
    // create_map_skelaton()
})

function create_map_skelaton() {
    var map = new L.Map("map", {center: [13.7, 100.5], zoom: 5, minZoom: 4, maxZoom: 12})
    map.addLayer(new L.TileLayer("http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"));
    // TODO add map.on("zoom")
    ...
    // add data
    draw_fires(map, "SELECT * FROM nasafirmdata order by acq_date")
}

function draw_fires(map, query, provinceData) {
    ...
    fetch('/getdata', {
        ...
        .then(coordinates => {
            ...
            let allGroup = d3.map(coordinates, function(d){return(d.acq_date);}); // This takes the entire column for the date
            let uniqueDates = allGroup.filter((item, i, ar) => ar.indexOf(item) === i);
            let dataForDate = coordinates.filter(function(d) {
                return d["acq_date"] === uniqueDates[0]
            });

            let svg1 = d3.select(map.getPanes().overlayPane).append("svg").attr("class", "circle-layer"),
            ...
            let allCircles = document.querySelectorAll(".inner-CL > circle");
            ...
            map.on("zoom", () => reset(map, dataForDate))
        })

}

function reset(map, dataForDate) {
    console.log("starting reset")
    let svg1 = document.getElementsByClassName("circle-layer")
    let g1 = document.getElementsByClassName('inner-CL')
...
function draw_provinces(map) {
    if(document.getElementById("province_json_data").innerText == '') {
        Promise.all([
            d3.json("thailand.json")])
            .then(([provinceData]) => {
                document.getElementById("province_json_data").innerText = JSON.stringify(provinceData)
                let provinces = new Array;
                let j;
                for (j = 0; j < provinceData.features.length; j++) {
                    provinces.push(provinceData.features[j]);
                }
                let geojson = L.geoJSON(provinces, {
                    style: style//,
